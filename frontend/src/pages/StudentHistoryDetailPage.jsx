    ...
    const fetchSubmissionDetails = async () => {
      ...
      try {
        const res = await axios.get(
          `http://localhost:3000/api/results/${submissionId}`
        );
        if (res.data) {
          setDetails(res.data);
        } else {
    ...
    return (
      <>
        ...
        <div className="history-detail-page-container error-message">
          ...
          <Link to="/history" className="back-to-history-list-btn">
            ‹ Quay lại Lịch sử
          </Link>
        </div>
        <Footer />
    ...
    <>
      ...
      <div className="history-detail-page-container">
        <div className="history-detail-header">
          <p>
            <strong>Mã bài kiểm tra:</strong> {code}
          </p>
          <div className="history-summary-stats">
            <span>
              <strong>Tổng số câu:</strong> {totalQuestions}
            </span>
            <span>
            ...
            <span>
              ...
              <span className="summary-incorrect">{incorrectCount}</span>
            </span>
            <span>
              <strong>Thời gian làm bài:</strong> {formatTimeSpent(timeSpent)}
            </span>
            <span>
              ...
              >
                ...
                <div className="history-answers-group">
                  {question.options.map((optionText, optIndex) => {
                    ...
                    const isStudentChoice = optionLetter === studentChoice;

                    const isSystemCorrectAnswer =
                    ...
                    return (
                      <div key={optIndex} className={optionClassName}>
                        ...
                        <span className="history-option-text">
                          {optionText}
                        </span>
                        {isStudentChoice && isCorrect && (
                          <span className="answer-status-badge correct-badge">
                            ✔
                          </span>
                        )}
                        {isStudentChoice && !isCorrect && (
                          <span className="answer-status-badge incorrect-badge">
                            ✘
                          </span>
                        )}
                      </div>
                ...
                {!isCorrect && userAnswer && (
                  <p className="system-correct-answer-feedback">
                    Đáp án đúng: {correctAnswerLetter}
                  </p>
                )}
                {!userAnswer && (
                  <p className="not-answered-feedback">
                    Không trả lời. Đáp án đúng: {correctAnswerLetter}.
                  </p>
                )}
              </div>
            );
